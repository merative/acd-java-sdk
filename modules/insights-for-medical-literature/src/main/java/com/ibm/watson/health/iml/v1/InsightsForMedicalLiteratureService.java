...
public class InsightsForMedicalLiteratureService extends BaseService {
  ...
  private static final String SERVICE_NAME =
    "insights_for_medical_literature_service";
   ...
   * Specifying this value will keep your API calls from failing when the
   * service introduces breaking changes.
   ...
   */
  public InsightsForMedicalLiteratureService(final String versionDate,
      final String url) {
    super(SERVICE_NAME, new NoAuthAuthenticator());
    HttpConfigOptions options = new HttpConfigOptions.Builder()
    		.disableSslVerification(true).build();
   ...
   */
  public InsightsForMedicalLiteratureService(final String versionDate,
		 final String apikey, final String url, HttpConfigOptions options) {
   ...
   */
  public InsightsForMedicalLiteratureService(final String versionDate,
		  final String apikey, final String iamUrl, final String url, final HttpConfigOptions options) {
    super(SERVICE_NAME,
   ...
   *
   * The response returns a list of metadata field names that
   * can be used by the POST search API.
   *
   * @param getFieldsOptions the {@link GetFieldsOptions}
   * containing the options for the call
   * @return a {@link ServiceCall} with a response type of
   * {@link MetadataModel}
   */
  public ServiceCall<MetadataModel> getFields(
		  final GetFieldsOptions getFieldsOptions) {
   ...
   *
   * @param searchOptions the {@link SearchOptions} containing the
  ...
  public ServiceCall<SearchModel> search(final SearchOptions searchOptions) {
    ...
    if (searchOptions.query() != null) {
      contentJson.add("query",
    		  GsonSingleton.getGson().toJsonTree(searchOptions.query()));
    }
    if (searchOptions.returns() != null) {
      contentJson.add("returns",
    		  GsonSingleton.getGson().toJsonTree(searchOptions.returns()));
    }
    builder.bodyJson(contentJson);
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(SearchModel.class));
  }

  /**
   * Find concepts matching the specified query string.
   *
   * Searches concepts mentioned in the corpus looking for matches on the
   * query string field. The comparison is not case
   * sensitive. The main use of this method is to build query boxes that
   * offer auto-complete, to allow users to select
   * valid concepts.
   *
   * @param typeaheadOptions the {@link TypeaheadOptions} containing the
   * options for the call
   * @return a {@link ServiceCall} with a response type of
   * {@link ConceptListModel}
   */
  public ServiceCall<ConceptListModel> typeahead(
		 final  TypeaheadOptions typeaheadOptions) {
    ...
    if (typeaheadOptions.ontologies() != null) {
      builder.query("ontologies", RequestUtils
    		.join(typeaheadOptions.ontologies(), ","));
    }
    ...
    if (typeaheadOptions.maxHitCount() != null) {
      builder.query("max_hit_count",
    		String.valueOf(typeaheadOptions.maxHitCount()));
    }
    if (typeaheadOptions.noDuplicates() != null) {
      builder.query("no_duplicates",
    		String.valueOf(typeaheadOptions.noDuplicates()));
    }
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(ConceptListModel.class));
  }

  /**
   * Retrieves information for concepts mentioned in this corpus.
   *
   * The response returns concepts mentioned in this corpus.  The returned
   *  concepts may be selected by CUI, preferred name, surface forms
   *  and attribute name.  All selected concepts are returned.
   *
   * @param getConceptsOptions the {@link GetConceptsOptions} containing
   *  the options for the call
   * @return a {@link ServiceCall} with a response type of
   *  {@link ConceptListModel}
   */
  public ServiceCall<ConceptListModel> getConcepts(
		  final GetConceptsOptions getConceptsOptions) {
    ...
    if (getConceptsOptions.preferredNames() != null) {
      builder.query("preferred_names",
    		getConceptsOptions.preferredNames().toArray());
    }
    if (getConceptsOptions.surfaceForms() != null) {
      builder.query("surface_forms",
    		getConceptsOptions.surfaceForms().toArray());
    }
   ...
   *
   * This method is only supported for custom plan instances that make use
   *  of custom analytics.
   * Each custom analytic artifact should be persisted in the service prior
   *  to adding content.
   *
   * @param addArtifactOptions the {@link AddArtifactOptions} containing
   *  the options for the call
   * @return a {@link ServiceCall} with a response type of Void
   */
  public ServiceCall<Void> addArtifact(
		  final AddArtifactOptions addArtifactOptions) {
    Validator.notNull(addArtifactOptions,
    ...
    if (addArtifactOptions.dictionaryEntry() != null) {
      contentJson.add("dictionaryEntry", GsonSingleton.getGson()
    		  .toJsonTree(addArtifactOptions.dictionaryEntry()));
    }
    if (addArtifactOptions.attributeEntry() != null) {
      contentJson.add("attributeEntry", GsonSingleton.getGson()
    		  .toJsonTree(addArtifactOptions.attributeEntry()));
    }
    builder.bodyJson(contentJson);
    ResponseConverter<Void> responseConverter =
   ...
   *
   * The following fields may be retrieved:
   *  Preferred name
   *  Semantic types
   *  Surface forms - Ontology Dictionary names for this concept
   *  Variants - Onotogy Metathesaurus names for this concept
   *  Typeahead - Names included in the typeahead for this concept
   *  Definition - Concept definition (if available)
   * The default is to return all fields.  Individual fields may be selected
   *  using the '_fields' query parameter.
   *
   * @param getCuiInfoOptions the {@link GetCuiInfoOptions} containing the
   *  options for the call
   * @return a {@link ServiceCall} with a response type of
   *  {@link UmlsConceptInfoModel}
   */
  public ServiceCall<ConceptInfoModel> getCuiInfo(
		  final GetCuiInfoOptions getCuiInfoOptions) {
    Validator.notNull(getCuiInfoOptions, "getCuiInfoOptions cannot be null");
    String[] pathSegments = {"v1/corpora", "concepts"};
    String[] pathParameters = {getCuiInfoOptions.corpus(),
    ...
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(ConceptInfoModel.class));
  }

  /**
   * Retrieves a count of the number of times a concept is mentioned
   *  in the corpus.
   *
   * The response returns the number of times a concept is mentioned
   *  (hit count) in the corpus.
   *
   * @param getHitCountOptions the {@link GetHitCountOptions} containing
   *  the options for the call
   * @return a {@link ServiceCall} with a response type of {@link HitCount}
   */
  public ServiceCall<HitCount> getHitCount(
		  final GetHitCountOptions getHitCountOptions) {
    ...
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(HitCount.class));
  }

  /**
   * Retrieve concepts related to a concept.
   *
   * Returns a list of related concepts mentioned in the specified corpus.
  ...
  public ServiceCall<RelatedConceptsModel> getRelatedConcepts(
    ...
    Validator.notNull(getRelatedConceptsOptions,
    ...
    builder.query("recursive",
    		String.valueOf(getRelatedConceptsOptions.recursive()));
    }
    if (getRelatedConceptsOptions.treeLayout() != null) {
    builder.query("tree_layout",
    		String.valueOf(getRelatedConceptsOptions.treeLayout()));
    }
    if (getRelatedConceptsOptions.maxDepth() != null) {
    builder.query("max_depth",
    		String.valueOf(getRelatedConceptsOptions.maxDepth()));
    }
    if (getRelatedConceptsOptions.maxHitCount() != null) {
    builder.query("max_hit_count",
    		String.valueOf(getRelatedConceptsOptions.maxHitCount()));
    }
    if (getRelatedConceptsOptions.verbose() != null) {
    builder.query("verbose",
    		String.valueOf(getRelatedConceptsOptions.verbose()));
    }
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(RelatedConceptsModel.class));
  }

  /**
   * Find similar concepts.
   *
   * The response returns a list of similar concepts.
   * All ontologies defined in the corpora are searched.
   * Similarity is determined by checking for overlapping surface forms.
   *   The results are sorted in descending order by hit count.
   *
   ...
   *  {@link ConceptListModel}
   */
  public ServiceCall<ConceptListModel> getSimilarConcepts(
		  final GetSimilarConceptsOptions getSimilarConceptsOptions) {
    Validator.notNull(getSimilarConceptsOptions,
    ...
    if (getSimilarConceptsOptions.returnOntologies() != null) {
    	builder.query("return_ontologies",
    			getSimilarConceptsOptions.returnOntologies().toArray());
    }
    ...
    ResponseConverter<ConceptListModel> responseConverter =
        ResponseConverterUtils.getValue(new com.google.gson.reflect
        		.TypeToken<ConceptListModel>() { }.getType());
    return createServiceCall(builder.build(), responseConverter);
  }
   ...
   *
   * @param getCorporaConfigOptions the {@link GetCorporaConfigOptions}
   *  containing the options for the call
   * @return a {@link ServiceCall} with a response type of
  ...
  public ServiceCall<CorporaConfigModel> getCorporaConfig(
    ...
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(CorporaConfigModel.class));
  }
   ...
   * The method stores the configuration of how content should
   * be analyzed for a corpus.  The method takes connectivity information for
   * IBM Cloud Database for Elastic, the name of the corpus,
   * analyzed and non-analyzed fields, and references to pre-populated custom
   * analytic artifact stores.
   *
   * The response returns whether the instance schema was properly created.
   * This API should be used for defining a custom corpus schema.
   * Example POST body:{
   *    userName : 'string',
   *    password : 'string'
   *    repositoryUri : 'uri'
   ...
   * @return a {@link ServiceCall} with a response type of Void
   */
  public ServiceCall<CorporaConfigModel> setCorpusSchema(
		  final SetCorpusSchemaOptions setCorpusSchemaOptions) {
    Validator.notNull(setCorpusSchemaOptions,
    ...
    if (setCorpusSchemaOptions.enrichmentTargets() != null) {
      contentJson.add("enrichmentTargets", GsonSingleton.getGson()
    		  .toJsonTree(setCorpusSchemaOptions.enrichmentTargets()));
    }
    if (setCorpusSchemaOptions.metadataFields() != null) {
      contentJson.add("metadataFields", GsonSingleton.getGson()
    		  .toJsonTree(setCorpusSchemaOptions.metadataFields()));
    }
    ...
    if (setCorpusSchemaOptions.references() != null) {
      contentJson.add("references", GsonSingleton.getGson()
    		  .toJsonTree(setCorpusSchemaOptions.references()));
    }
    builder.bodyJson(contentJson);
   ...
   * @return a {@link ServiceCall} with a response type of Void
   */
  public ServiceCall<CorporaConfigModel> deleteCorpusSchema(
		  final DeleteCorpusSchemaOptions deleteCorpusSchemaOptions) {
    Validator.notNull(deleteCorpusSchemaOptions,
   ...
   *
   * This method is only supported for custom plan instances.
   * The method takes connection information for IBM Cloud Database for Elastic
   *  instances that will host custom corpora.
   *
   * The response returns whether the service successfully connected
   * to the specified repository
   * This API should be used for providing a custom enriched corpus.
   * Example POST body:{
   *    userName : 'string',
   *    password : 'string'
   *    repositoryUri : 'uri'
   * }
   *
   * @param setCorpusConfigOptions the {@link SetCorpusConfigOptions}
   *  containing the options for the call
   * @return a {@link ServiceCall} with a response type of Void
   */
  public ServiceCall<CorporaConfigModel> setCorpusConfig(
		  final SetCorpusConfigOptions setCorpusConfigOptions) {
    Validator.notNull(setCorpusConfigOptions,
   ...
   * enable monitoring.
   *
   * @param monitorCorpusOptions the {@link MonitorCorpusOptions}
   *  containing the options for the call
   * @return a {@link ServiceCall} with a response type of Void
   */
  public ServiceCall<Void> monitorCorpus(
		  final MonitorCorpusOptions monitorCorpusOptions) {
    Validator.notNull(monitorCorpusOptions,
   ...
   *
   * @param getCorpusConfigOptions the {@link GetCorpusConfigOptions}
   *  containing the options for the call
   * @return a {@link ServiceCall} with a response type of
   *  {@link CorporaConfigModel}
   */
  public ServiceCall<CorporaConfigModel> getCorpusConfig(
		  final GetCorpusConfigOptions getCorpusConfigOptions) {
    Validator.notNull(getCorpusConfigOptions,
   ...
   *
   * This method will always return 500 error if the service state is not OK.
   * This makes it simpler for service front ends to detect a failed service.
   *
  ...
  public ServiceCall<ServiceStatus> getHealthCheckStatus(
    ...
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(ServiceStatus.class));
  }
   ...
   *
   * The response returns a list of all the annotations contained
   *  in the document.
   *
   * @param getDocumentAnnotationsOptions the
   * {@link GetDocumentAnnotationsOptions} containing the options
   *  for the call
   * @return a {@link ServiceCall} with a response type of Void
   */
  public ServiceCall<AnnotationsModel> getDocumentAnnotations(
    ...
    Validator.notNull(getDocumentAnnotationsOptions,
      "getDocumentAnnotationsOptions cannot be null");
    String[] pathSegments = {"v1/corpora", "documents", "annotations"};
    String[] pathParameters = {getDocumentAnnotationsOptions.corpus(),
    		getDocumentAnnotationsOptions.documentId()};
    RequestBuilder builder = RequestBuilder.get(RequestBuilder
    		.constructHttpUrl(getServiceUrl(), pathSegments, pathParameters));
    builder.query(VERSION, versionDate);
    ...
    builder.query("cuis",
    		RequestUtils.join(getDocumentAnnotationsOptions.cuis(), ","));
    }
    if (getDocumentAnnotationsOptions.includeText() != null) {
    builder.query("include_text",
    		String.valueOf(getDocumentAnnotationsOptions.includeText()));
    }
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(AnnotationsModel.class));
  }

  /**
   * Categorizes concepts in a document.
   *
   * The response returns a categorized list of text passages in a document.
   * The sentences are grouped by concept with the matching concepts
   * highlighted (tagged).
   *
   * @param getDocumentCategoriesOptions the
   * {@link GetDocumentCategoriesOptions} containing the options for the call
   * @return a {@link ServiceCall} with a response type of
   * {@link CategoriesModel}
   */
  public ServiceCall<CategoriesModel> getDocumentCategories(
		  final GetDocumentCategoriesOptions getDocumentCategoriesOptions) {
    Validator.notNull(getDocumentCategoriesOptions,
    		"getDocumentCategoriesOptions cannot be null");
    String[] pathSegments = {"v1/corpora", "documents", "categories"};
    String[] pathParameters = {getDocumentCategoriesOptions.corpus(),
    		getDocumentCategoriesOptions.documentId()};
    RequestBuilder builder = RequestBuilder.get(RequestBuilder
    		.constructHttpUrl(getServiceUrl(), pathSegments, pathParameters));
    builder.query(VERSION, versionDate);
    ...
    builder.query("highlight_tag_end",
    		getDocumentCategoriesOptions.highlightTagEnd());
    }
    if (getDocumentCategoriesOptions.types() != null) {
    builder.query("types",
    		RequestUtils.join(getDocumentCategoriesOptions.types(), ","));
    }
    ...
    builder.query("only_negated_concepts",
    		String.valueOf(getDocumentCategoriesOptions
    				.onlyNegatedConcepts()));
    }
    ...
    builder.query("_limit",
    		String.valueOf(getDocumentCategoriesOptions.limit()));
    }
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(CategoriesModel.class));
  }

  /**
   * Retrieves the external ID, title, abstract and text for a document.
   *
   * The response may return the following fields:
   * external ID (e.g., PubMed ID)
   * title
   * abstract
   * bodypdfUrlreferenceUrl
   * other
   * metadata
   * Note, some documents may not have an abstract, or only the abstract may
   *  be available without the body text.
   *
   ...
   */
  public ServiceCall<DocumentTextModel> getDocumentInfo(
		  final GetDocumentInfoOptions getDocumentInfoOptions) {
    Validator.notNull(getDocumentInfoOptions,
    ...
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(DocumentTextModel.class));
  }

  /**
   * Categorizes concepts in a document.
   *
   * The response returns multiple categorized lists of text passages
   *  in a document.  The sentences are grouped by concept with the
   * matching concepts highlighted.<P>This API should be used to batch
   *  multiple categories in a single request to improve performance.
   *  Example POST body:{
   *  categories: [
   *   {
   *    name : 'disorders',
   ...
   *  {@link GetDocumentMultipleCategoriesOptions} containing the options
   *  for the call
   * @return a {@link ServiceCall} with a response type of
   *  {@link CategoriesModel}
   */
  public ServiceCall<MultipleCategoriesModel> getDocumentMultipleCategories(
		  final GetDocumentMultipleCategoriesOptions
		  getDocumentMultipleCategoriesOptions) {
    Validator.notNull(getDocumentMultipleCategoriesOptions,
    ...
    builder.query("highlight_tag_end",
    		getDocumentMultipleCategoriesOptions.highlightTagEnd());
    }
    ...
    builder.query("_limit",
    		String.valueOf(getDocumentMultipleCategoriesOptions.limit()));
    }
    final JsonObject contentJson = new JsonObject();
    if (getDocumentMultipleCategoriesOptions.categories() != null) {
      contentJson.add("categories", GsonSingleton.getGson()
    		  .toJsonTree(getDocumentMultipleCategoriesOptions.categories()));
    }
    builder.bodyJson(contentJson);
    return createServiceCall(builder.build(),
    		ResponseConverterUtils.getObject(MultipleCategoriesModel.class));
  }
   ...
   *   corpus provider.
   *
   * @param getDocumentsOptions the {@link GetDocumentsOptions}
   *  containing the options for the call
   * @return a {@link ServiceCall} with a response type of
   *  {@link CorpusInfoModel}
   */
  public ServiceCall<CorpusInfoModel> getDocuments(
		  final GetDocumentsOptions getDocumentsOptions) {
    Validator.notNull(getDocumentsOptions,
   ...
   *
   * This method is only supported for custom plan instances.
   * The method is used to add content that will be
   * analyzed to a corpus.
   *
   * The response returns whether the document was properly
   *  added to the corpus.
   * This API should be used for adding a document to a custom corpus.
   * Example POST body:{
   *   \"acdUrl\" :
   *   \"acdApiKeyl\" :
   *   \"flowId\" :
   ...
   * @return a {@link ServiceCall} with a response type of Void
   */
  public ServiceCall<Void> addCorpusDocument(
		  final AddCorpusDocumentOptions addCorpusDocumentOptions) {
    Validator.notNull(addCorpusDocumentOptions,
    ...
    if (addCorpusDocumentOptions.document() != null) {
      contentJson.add("document", GsonSingleton.getGson()
    		  .toJsonTree(addCorpusDocumentOptions.document()));
    }
    ...
    if (addCorpusDocumentOptions.accessToken() != null) {
      contentJson.addProperty("accessToken",
    		  addCorpusDocumentOptions.accessToken());
    }
    if (addCorpusDocumentOptions.otherAnnotators() != null) {
      contentJson.add("otherAnnotators", GsonSingleton.getGson()
    		  .toJsonTree(addCorpusDocumentOptions.otherAnnotators()));
    }
    builder.bodyJson(contentJson);
    ResponseConverter<Void> responseConverter =
    		ResponseConverterUtils.getVoid();
   ...
   *
   * Returns matching concepts and text passages. The sentences containing
   * each concept are returned with the concept highlighted.
   * Extended annotations provide additional details for  each discrete
   * search match detected in the document.
   * An iml-annotation-id attribute is added to each highlight tag which
   ...
   */
  public ServiceCall<SearchMatchesModel> getSearchMatches(
		  final GetSearchMatchesOptions getSearchMatchesOptions) {
    Validator.notNull(getSearchMatchesOptions,
    		"getSearchMatchesOptions cannot be null");
    String[] pathSegments = { "v1/corpora", "documents", "search_matches" };
    String[] pathParameters = { getSearchMatchesOptions.corpus(),
    		getSearchMatchesOptions.documentId() };
    RequestBuilder builder = RequestBuilder.get(RequestBuilder
    		.constructHttpUrl(getServiceUrl(), pathSegments, pathParameters));
    builder.query(VERSION, versionDate);
    ...
    builder.query("attributes",
    		getSearchMatchesOptions.attributes().toArray());
    }
    ...
    builder.query("search_tag_begin",
    		getSearchMatchesOptions.searchTagBegin());
    }
   ...
   */
    public static final class Builder {
        ...
        /**
         * Builder versionDate method to configure the IML REST APIs version of the client.
         *
         * @param the version date string as YYYY-MM-DD
         */
		...
		public InsightsForMedicalLiteratureService build() {
			if (apikey == null) {
				...
				if (iamUrl == null) {
					...
					return new InsightsForMedicalLiteratureService(versionDate,
							apikey, iamUrl, url, httpOptions);
				}
			}
		}
    }
